...
function cleanRawPayload(payload: DataLayerPayload) {
  return Object.fromEntries(
...
export function pushRawDataLayerEvent(
  event: string,
  payload: DataLayerPayload = {}
) {
  if (typeof window === "undefined") {
  ...
  window.dataLayer.push({
    event,
    ...cleanRawPayload(payload),
  })
}

export function pushDedupedRawDataLayerEvent(
  event: string,
  payload: DataLayerPayload,
  dedupeKey: string
) {
  ...
  pushRawDataLayerEvent(event, payload)
}
