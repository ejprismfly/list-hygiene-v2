...
test("documented workspace roles SQL matches the Supabase migration source", () => {
  const sql = read("supabase/migrations/20260723000000_workspace_roles.sql")

  assert.equal(read("docs/migration/sql/20260723_workspace_roles.sql"), sql)
  assert.match(sql, /workspace_members_one_owner_per_workspace_idx/)
  assert.match(sql, /where role = 'owner'/)
  assert.match(sql, /transfer_workspace_ownership/)
  assert.match(sql, /recalculate_organization_member_role/)
  assert.match(sql, /can_manage_workspace/)
...
test("workspace/billing/integration API surface exists in v2 app router", () => {
  const requiredRoutes = [
    ...
    "src/app/api/workspaces/ownership/route.ts",
...
test("side-by-side deployment docs capture live database constraints", () => {
  ...
  assert.match(readiness, /20260723_workspace_roles\.sql/)
...
test("direct signup enters workspace onboarding while invites keep invite flow", () => {
  ...
  const onboardingPage = read("src/app/(standalone)/onboarding/page.tsx")
  const onboardingContent = read("src/components/app/onboarding-content.tsx")
  const onboarding = read("src/lib/onboarding.ts")
  const onboardingServer = read("src/lib/onboarding-server.ts")
  const klaviyoOAuth = read("src/lib/klaviyo-oauth.ts")
  ...
  assert.match(onboarding, /SIGNUP_ONBOARDING_COOKIE/)
  assert.match(onboarding, /lh_signup_onboarding/)
  assert.match(onboarding, /isOnboardingPath/)
  assert.match(onboardingServer, /shouldShowOnboardingForUser/)
  assert.match(onboardingServer, /userHasKlaviyoIntegrationHistory/)
  assert.match(onboardingServer, /\.from\("klaviyo_accounts"\)/)
  assert.match(onboardingServer, /\.eq\("user_id", userId\)/)
  assert.match(onboardingServer, /\.from\("workspace_members"\)/)
  assert.match(onboardingServer, /\.in\("workspace_id", accessibleWorkspaceIds\)/)
  assert.match(onboardingServer, /\.is\("workspace_id", null\)/)
  assert.match(dashboardPage, /shouldShowOnboardingForUser\(user\)/)
  ...
  assert.match(onboardingPage, /hasNoIntegrationHistory/)
  assert.match(onboardingPage, /redirect\("\/dashboard"\)/)
  assert.match(onboardingContent, /clearSignupOnboardingMarker/)
  assert.match(onboardingContent, /SIGNUP_ONBOARDING_COOKIE/)
...
test("team invitations are usable by existing and new users", () => {
  ...
  const invitationsRoute = read("src/app/api/organizations/invitations/route.ts")
  const ownershipRoute = read("src/app/api/workspaces/ownership/route.ts")
  ...
  assert.match(workspaceSwitcher, /Transfer ownership before removing the workspace owner\./)
  assert.match(workspaceSwitcher, /managerEnabled = canManage\(selectedWorkspace\?\.role\)/)
  assert.match(workspaceSwitcher, /ownerEnabled = selectedWorkspace\?\.role === "owner"/)
  assert.match(workspaceSwitcher, /<DialogTitle>Transfer ownership<\/DialogTitle>/)
  assert.match(workspaceSwitcher, /\/api\/workspaces\/ownership/)
  ...
  assert.match(workspaceSwitcher, /invitation\.status === "pending" \|\| invitation\.status === "expired"/)
  ...
  assert.match(memberRoute, /Transfer ownership before changing the workspace owner\./)
  ...
  assert.match(memberRoute, /At least one workspace admin or owner is required\./)
  ...
  assert.match(teamMembers, /Team members must have at least one workspace\./)
  assert.match(teamMembers, /organization_invitations/)
  assert.match(teamMembers, /status: "accepted"/)
  assert.match(ownershipRoute, /canOwnWorkspace/)
  assert.match(ownershipRoute, /signInWithPassword/)
  assert.match(ownershipRoute, /transfer_workspace_ownership/)
  assert.match(ownershipRoute, /new_owner_user_id/)
  ...
  assert.match(invitationsRoute, /canManageWorkspace/)
  assert.match(invitationsRoute, /invitationWithDerivedStatus/)
  assert.match(invitationsRoute, /\.in\("status", \["pending", "expired"\]\)/)
...
test("billing manage opens Stripe portal and plan actions open checkout", () => {
  ...
  assert.match(billingContent, /canManageBilling = Boolean\(billing\.permissions\?\.can_manage_billing\)/)
  assert.match(billingContent, /\(loading \|\| canManageBilling\) &&/)
  assert.match(billingContent, /Only owners and admins can manage billing\./)
  ...
  assert.match(billingRoute, /canManageBilling\(billing\.context\.tenant\?\.role \?\? null\)/)
  assert.match(billingRoute, /\$\{billingHost\}\/api\/billing\/checkout/)
  assert.match(billingRoute, /portal: canManage \? `\$\{billingHost\}\/api\/billing\/portal` : null/)
  assert.match(billingRoute, /can_manage_billing: canManage/)
  ...
  assert.match(plansRoute, /canManageBilling\(billing\.context\.tenant\?\.role \?\? null\)/)
  ...
  assert.match(portalRoute, /canManageBilling\(billing\.context\.tenant\?\.role \?\? null\)/)
  assert.match(portalRoute, /Only owners and admins can manage billing/)
  ...
  assert.match(checkoutRoute, /canManageBilling\(billing\.context\.tenant\?\.role \?\? null\)/)
  ...
  assert.match(customerRoute, /canManageBilling\(billing\.context\.tenant\?\.role \?\? null\)/)
  ...
  assert.match(paymentRoute, /canManageBilling\(billing\.context\.tenant\?\.role \?\? null\)/)
...
test("browser billing GTM tracking only fires paid subscription after Stripe verification", () => {
  const env = read(".env.example")
  const layout = read("src/app/layout.tsx")
  const gtm = read("src/components/app/google-tag-manager.tsx")
  const googleTag = read("src/components/app/google-analytics-tag.tsx")
  ...
  const billingFailed = read("src/app/(app)/billing/failed/page.tsx")
  const billingCustomer = read("src/app/api/billing/customer/route.ts")
  const signupForm = read("src/components/auth/signup-form.tsx")
  const signupActions = read("src/app/(auth)/actions.ts")

  assert.match(env, /^NEXT_PUBLIC_GTM_ID=GTM-N7976269/m)
  assert.match(env, /^NEXT_PUBLIC_GOOGLE_TAG_ID=G-HW48Q6PPY4/m)
  assert.match(layout, /GoogleTagManager,\s*[\s\S]*GoogleTagManagerNoScript/)
  assert.match(layout, /GoogleTagManagerNoScript/)
  assert.match(layout, /<head>[\s\S]*<GoogleTagManager \/>[\s\S]*<\/head>/)
  assert.match(layout, /<body[\s\S]*<GoogleTagManagerNoScript \/>/)
  assert.match(layout, /import \{ GoogleAnalyticsTag \}/)
  assert.match(layout, /<\/head>[\s\S]*<body[\s\S]*<GoogleAnalyticsTag \/>/)
  assert.match(gtm, /NEXT_PUBLIC_GTM_ID/)
  assert.match(gtm, /GTM-N7976269/)
  assert.match(gtm, /googletagmanager\.com\/gtm\.js/)
  assert.match(gtm, /strategy="beforeInteractive"/)
  assert.match(gtm, /dataLayer/)
  assert.match(gtm, /GoogleTagManagerNoScript/)
  assert.match(gtm, /googletagmanager\.com\/ns\.html/)
  assert.match(gtm, /display: "none", visibility: "hidden"/)
  assert.match(googleTag, /NEXT_PUBLIC_GOOGLE_TAG_ID/)
  assert.match(googleTag, /G-HW48Q6PPY4/)
  assert.match(googleTag, /googletagmanager\.com\/gtag\/js/)
  assert.match(googleTag, /function gtag\(\)\{dataLayer\.push\(arguments\);\}/)
  assert.match(googleTag, /gtag\("config", \$\{googleTagIdJson\}\)/)
  assert.match(analytics, /pushDataLayerEvent/)
  assert.match(analytics, /pushDedupedDataLayerEvent/)
  assert.match(analytics, /pushRawDataLayerEvent/)
  assert.match(analytics, /pushDedupedRawDataLayerEvent/)
  ...
  assert.match(billingTracking, /paid_subscription/)
  assert.match(billingTracking, /transaction_id/)
  assert.match(billingTracking, /transaction_value/)
  assert.match(billingTracking, /transaction_currency/)
  assert.match(billingTracking, /customer_email/)
  ...
  assert.match(returnTracker, /\/api\/billing\/customer\$\{query\}/)
  assert.match(returnTracker, /session_id/)
  assert.match(returnTracker, /paidSubscriptionFromResponse/)
  assert.match(returnTracker, /status === "success" && !paidSubscription/)
  assert.match(returnTracker, /trackPaidSubscription\(paidSubscription\)/)
  assert.match(returnTracker, /params\.get\("cancel"\) === "true"/)
  assert.match(billingFailed, /<BillingReturnTracker status="failed" \/>/)
  assert.match(billingCustomer, /checkout\.sessions\.retrieve\(sessionId/)
  assert.match(billingCustomer, /session\.payment_status !== "paid"/)
  assert.match(billingCustomer, /session\.mode !== "subscription"/)
  assert.match(billingCustomer, /amountPaidCents \/ 100/)
  assert.match(billingCustomer, /paid_subscription: paidSubscription/)
  assert.doesNotMatch(signupForm, /paid_subscription|dataLayer|trackPaidSubscription/)
...
test("critical UI controls are wired to their own actions", () => {
  ...
  assert.match(workspaceSwitcher, /aria-describedby=\{[\s\S]*inviteStatusMessage[\s\S]*"member-email-feedback"[\s\S]*undefined[\s\S]*\}/)
...
test("Klaviyo OAuth routes are workspace-scoped and maintain token lifecycle", () => {
  ...
  assert.match(callback, /canCreateIntegrations/)
  assert.match(callback, /resolveTenantContext\(request, \{ requireWorkspace: true \}\)/)
  assert.match(accounts, /applyAccountScope/)
  assert.match(accounts, /canUpdateIntegrations/)
  assert.match(accounts, /resolveTenantContext\(request, \{ requireWorkspace: true \}\)/)
  ...
  assert.match(segments, /canUpdateIntegrations/)
  assert.match(segments, /resolveTenantContext\(request, \{ requireWorkspace: true \}\)/)
  ...
  assert.match(disconnect, /canDeleteIntegrations/)
  assert.match(disconnect, /Only owners and admins can delete integrations/)
  assert.match(disconnect, /resolveTenantContext\(request, \{ requireWorkspace: true \}\)/)
...
test("new workspaces seed the creator as workspace owner", () => {
  const route = read("src/app/api/workspaces/route.ts")

  assert.match(route, /Only owners and admins can create workspaces/)
  assert.match(route, /created_by_user_id/)
  assert.match(route, /role: "owner"/)
  assert.match(route, /member_count: 1/)
  assert.match(route, /set role = 'owner'/)
})

test("login and logout clear previous user client state", () => {
  const loginForm = read("src/components/auth/login-form.tsx")
  const logoutForm = read("src/components/app/logout-form.tsx")
  const authClientState = read("src/lib/auth-client-state.ts")
  const authActions = read("src/app/(auth)/actions.ts")
  const desktopShell = read("src/components/app/app-shell.tsx")
  const mobileShell = read("src/components/app/mobile-menu.tsx")
  const profileContent = read("src/components/profile/profile-content.tsx")
  ...
  assert.match(logoutForm, /action=\{signOutAction\}/)
  assert.match(authClientState, /clearWorkspaceClientState\(window\.localStorage\)/)
  assert.match(authClientState, /invalidateWorkspaceClientData\(\)/)
  assert.match(authClientState, /clearSupabaseAuthStorage\(window\.localStorage\)/)
  assert.match(authClientState, /clearSupabaseAuthStorage\(window\.sessionStorage\)/)
  assert.match(authClientState, /key\.startsWith\("sb-"\) && key\.includes\("auth-token"\)/)
  assert.match(authActions, /await clearPreviousAuthCookies\(\)[\s\S]*const supabase = await createClient\(\)[\s\S]*signInWithPassword/)
  assert.match(authActions, /isSupabaseAuthCookie/)
