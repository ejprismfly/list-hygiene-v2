...
test("team invitations are usable by existing and new users", () => {
  ...
  assert.match(workspaceSwitcher, /email_delivery\?: "existing_user" \| "supabase_auth"/)
  ...
  assert.match(invitationsRoute, /acceptedInvitationId: invitation\.id/)
  assert.match(invitationsRoute, /inviteUserByEmail/)
  assert.match(invitationsRoute, /action === "resend"/)
  assert.match(invitationsRoute, /resent: true/)
  assert.match(invitationsRoute, /buildInviteAuthRedirectUrl/)
  assert.match(invitationsRoute, /email_delivery: "supabase_auth"/)
