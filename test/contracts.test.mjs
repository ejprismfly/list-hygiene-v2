...
test("critical UI controls are wired to their own actions", () => {
  ...
  assert.match(settingsContent, /popup=yes/)
  assert.match(settingsContent, /width=\$\{width\}/)
  assert.match(settingsContent, /height=\$\{height\}/)
  assert.match(settingsContent, /"klaviyo-oauth"/)
  assert.match(settingsContent, /popup\.location\.href = authUrl/)
  assert.match(settingsContent, /popup\.focus\(\)/)
  ...
  assert.match(settingsContent, /<div className="overflow-x-auto">\s*<Table className="min-w-\[44rem\]">/)
